...
class ZmqDiagnostics:
    ...
    def open_zmq(self):
        ...
        if not self.is_socket_open:
            if self.port == '':
                self.logger.error('ZMQ Diagnostics port to listen to has not been defined')
            else:
                try:
    ...
    def display_zmq_messages(self):
        ...
        :return:
        """
        self.logger.debug('ZMQ Diagnostic listening to tcp://%s:%s' % (self.zmq_publisher_address, self.port))
        while True:
